...
`package.json` 의 **직접 (non-dev) 의존성 ≤ 15개**. 모든 직접 의존은 CHARTER §6 supply-chain 표에 대응. 새 의존 추가는 사유 + `npm audit` clean + 코드 검토 후. `<script src=>` / runtime CDN import 0개. node_modules 의 postinstall script 는 `npm ci --ignore-scripts` 로 차단.
...
### VII. Device Hash Confirmation (Ledger)

Ledger WebHID 서명 시 device 화면이 보여주는 domain hash / message hash 와 SPA UI 가 보여주는 값이 **완전히 일치하는지** 사용자가 확인하는 단계를 UI 가 강제한다. 단순 텍스트 안내가 아니라 modal 의 typed-confirm 체크박스. CHARTER §6 의 host-탈취 위협 mitigation.
...
- V. `package.json` 의 `dependencies` 항목 수 ≤ 15.
- VI. `tests/golden/` 디렉토리 존재 + 최소 fixture 파일 1개.
- VII. Ledger 서명 경로에 `confirmDeviceHash` modal 호출 grep.
...
본 헌법은 hl-vote-web 의 모든 design / code / PR 결정에 우선한다. 위반 시 `plan.md` 의 Complexity Tracking 표에 사유 + builnad 명시 승인. **슬래시 위험과 직결되는 원칙 (II, III, IV, VI, VII, IX)** 은 예외 없음.
